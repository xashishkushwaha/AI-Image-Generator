...
const ImageCard = ({ item, index, setImageDetails }) => {
  const [like, setLike] = useState(null);
  return (
    <div onClick={() => setImageDetails(item)} className="new-Card-width">
      <div
        className={`block relative group select-none overflow-hidden m-0.5 border-indigo-600 cursor-pointer rounded-tl-2xl rounded-tr-2xl
        }   `}
        style={{ transition: "opacity 250ms ease 0s" }}
      >
          ...
          >
            <div className="flex-shrink h-full flex items-end">
              <div className="flex flex-col">
                ...
                >
                  Cyril Ramaphosa scary hairy humanoid smoking a tobacco on the
                  tree top
                </p>
                <p
                ...
                >
                  Forest, foggy weather, night light, intricate, sharf focus,
                  conc...
                </p>
              </div>
            </div>
        ...
        <div className="absolute top-0 left-0 right-0 p-2 flex z-10 text-zinc-100 justify-between pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity  text-sm">
          <a
            className="bg-zinc-900 bg-opacity-50 hover:bg-opacity-100 transition-opacity flex items-center justify-center pointer-events-auto cursor-pointer text-lg h-10 w-10 rounded-lg"
            href="/?q=94cb9b66-12b3-4fc5-a5ef-2cf6c4b1413a"
          >
            <ImgSearch />
          </a>
        ...
        <img className="pointer-events-none" src={item?.image} />
      </div>
    </div>
  );
};
